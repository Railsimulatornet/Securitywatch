...
def build_text(findings: List[Dict[str, Any]], sources: List[Dict[str, Any]], raw_total: int, lang: str) -> str:
...
def build_html(findings: List[Dict[str, Any]], sources: List[Dict[str, Any]], raw_total: int, lang: str) -> str:
    overall_critical = sum(1 for f in findings if f["severity"] == "CRITICAL")
    overall_high = sum(1 for f in findings if f["severity"] == "HIGH")
    duplicates_removed = max(raw_total - len(findings), 0)
...
def main() -> None:
    ...
    Path(args.mail_txt).write_text(build_text(findings, sources, raw_total, lang), encoding="utf-8")
    Path(args.mail_html).write_text(build_html(findings, sources, raw_total, lang), encoding="utf-8")
